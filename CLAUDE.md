...
| `/code/:code` | CodeRedeem.tsx | Public reusable admin-issued access code (no auth, no purchase) |
...
- **access_codes** — id, code (`citext`, unique and case-insensitive), video_id (FK videos), note (**user-visible** — rendered on the redemption page under the video title, not an admin memo), redeemed_at (last redemption timestamp), redeemed_views (redemption counter, analytics only), created_at. Admin-issued **reusable** codes for free viewing of a specific video — no auth, no purchase required, no view cap. Counter increments run through the `bump_access_code_usage(uuid)` RPC so concurrent redemptions don't lose updates. RLS is enabled with **no public policies**; all reads/writes go through the `redeem-code` edge function (service_role). Mint via Supabase MCP. To retire a code, delete its row. Distinct from `video_gifts` (which require a prior purchase and are single-use buyer→friend).
...
- **redeem-code** — Public (`verify_jwt: false`, no auth). Given a `{code}`, looks up the row in `access_codes` with `.eq('code', code)` (case-insensitive via `citext`). Codes are **reusable** — every call returns `{video_url, video_title, note}` and bumps `redeemed_views` + `redeemed_at` atomically via the `bump_access_code_usage` RPC as best-effort analytics (failures logged, not surfaced). Errors: `code_not_found`, `video_not_available`. `note` is user-visible — write it as friendly copy for the viewer, not as an admin memo.
...
- Access-code flow — admin-issued reusable codes in `access_codes`, public redemption at `/code/:code` via `redeem-code` edge function (no auth, no purchase, no view cap). Minted via Supabase MCP. Current live code: `redeem-your-free-video` → *Rent, Food, or Future?*.
...
- **Minting an access code** — Insert into `access_codes` via Supabase MCP: `INSERT INTO access_codes (code, video_id, note) VALUES ('your-code', '<video_uuid>', 'A complimentary viewing — enjoy!')`. The code is matched case-insensitively (`citext`). **`note` is user-visible** — it renders under the video title on the redemption page, so write it as friendly copy for the viewer (e.g. "A complimentary viewing from Primrose Watson — enjoy!"), never as an admin memo. Share the URL `${SITE_URL}/code/your-code` — anyone with the link can watch the video for free, no sign-in, no purchase. Codes are **reusable** (no view cap) — `redeemed_views` is a running analytics counter and `redeemed_at` is the last-redemption timestamp. To retire a code, `DELETE FROM access_codes WHERE code = 'your-code'`.
...
src/
  ...
  pages/
    ...
    CodeRedeem.tsx     # Public /code/:code single-use access code redemption
public/
...
supabase/
  ...
  functions/
    ...
    redeem-code/index.ts       # Public: single-use access code redemption
